...
export function ChatSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`hidden ${
          collapsed ? "md:hidden" : "md:flex md:w-55 md:shrink-0"
        } md:flex-col border-r bg-sidebar text-sidebar-foreground`}
        aria-label="Chat sidebar"
      >
        <div className="px-4 py-2">
          <Button
            onClick={newChat}
            className="w-full text-sidebar-foreground bg-sidebar hover:bg-sidebar-accent"
          >
          ...
          <nav className="p-2 space-y-1">
            {sorted.length === 0 ? (
              ...
              sorted.map((s) => (
                <div
                  key={s.id}
                  className={`flex items-center gap-1 px-1 group transition-colors ${
                    activeChatId === s.id
                      ? "bg-card"
